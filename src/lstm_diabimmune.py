...
import src.Ordered_Neurons_LSTM as oredered_lstm
...
def dynamicRNN(x, seqlen, weights, biases, seq_max_len, n_hidden):
    ...
    on_lstm = oredered_lstm.ON_LSTM(num_units=n_hidden, chunk_size=8)
...
def trainingLSTM(maxLen, numFeatures, subjects, meta, timepoints, df):
    ...
    print("Average Precision = %0.2f,   Average Recall = %0.2f" % (np.mean(testPrecision), np.mean(testRecall)))
    return
...
def main(_):
    ...
    maxLen, numFeatures, subjects, meta, timepoints, df = utils.lstm_raw_input(MetadataFile, InputFile)
    # maxLen, numFeatures, subjects, meta, timepoints, df = utils.lstm_latent_input(MetadataFile, InputFile)
